...
> 💡 **KRX 정보데이터시스템 연동 (권장)**
> 지수 구성 정보를 한국거래소(KRX)로부터 공식적이고 정확하게 받아오려면 실행 전 `KRX_ID` 및 `KRX_PW` 환경 변수를 입력해 주세요. (입력하지 않으면 네이버 금융을 통한 웹 크롤러 방식으로 자동 대체됩니다.)
>
> ```bash
> # Linux / macOS
> export KRX_ID="본인의_KRX_아이디"
> export KRX_PW="본인의_KRX_비밀번호"
> 
> # Windows (CMD)
> set KRX_ID=본인의_KRX_아이디
> set KRX_PW=본인의_KRX_비밀번호
> ```
...
```
*(성공적으로 실행되면 `stock_data.db` 파일이 생성되고 `tickers` 및 `us_tickers` 테이블이 채워집니다. 또한 최신 지수 구성종목이 `index_constituents.json` 파일에 저장됩니다.)*
